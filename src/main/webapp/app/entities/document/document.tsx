...
import { getEntities, searchEntities } from './document.reducer';
  ...
  return (
    <div>
      <h2 id="document-heading" data-cy="DocumentHeading">
        <Translate contentKey="rentcarappjhipApp.document.home.title">Documents</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="rentcarappjhipApp.document.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/document/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="rentcarappjhipApp.document.home.createLabel">Create new Document</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('rentcarappjhipApp.document.home.search')}
                />
        ...
        {documentList && documentList.length > 0 ? (
            ...
            <thead>
              <tr>
                <th className="hand" onClick={sort('id')}>
                  <Translate contentKey="rentcarappjhipApp.document.id">ID</Translate>{' '}
                ...
                <th className="hand" onClick={sort('givenDate')}>
                  <Translate contentKey="rentcarappjhipApp.document.givenDate">Given Date</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('givenDate')} />
                </th>
                <th className="hand" onClick={sort('issuedDate')}>
                  ...
                  <FontAwesomeIcon icon={getSortIconByFieldName('issuedDate')} />
                </th>
                <th className="hand" onClick={sort('docStatus')}>
                  <Translate contentKey="rentcarappjhipApp.document.docStatus">Doc Status</Translate>{' '}
                  <FontAwesomeIcon icon={getSortIconByFieldName('docStatus')} />
                </th>
                <th />
              </tr>
            </thead>
            <tbody>
              {documentList.map((document, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  ...
                  <td>{document.name}</td>
                  <td>
                    <Translate contentKey={`rentcarappjhipApp.DocTypeEnum.${document.docType}`} />
                  </td>
                  <td>{document.givenDate ? <TextFormat type="date" value={document.givenDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{document.issuedDate ? <TextFormat type="date" value={document.issuedDate} format={APP_DATE_FORMAT} /> : null}</td>
                  <td>{document.docStatus ? 'true' : 'false'}</td>
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="rentcarappjhipApp.document.home.notFound">No Documents found</Translate>
            </div>
          )
        )}
      </div>
